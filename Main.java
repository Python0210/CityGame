...
class Main {
  ...
  private static void gameLoop() {
    ...
    while (true) {
      ...
      if (!isCorrectCity) {
        System.out.println("Такого міста не існує в нашій базі данних. Введіть інше місто");
      }

      if (lastComputerCity != null) {
        char lastComputerCityChar = lastComputerCity.charAt(lastComputerCity.length() - 1);
        // Перевіряємо на ь, и, й
        if (lastComputerCityChar == 'ь' || lastComputerCityChar == 'и' || lastComputerCityChar == 'й') {
          char lastComputerCityChar2 = lastComputerCity.charAt(lastComputerCity.length() - 2);

          char firstInputChar = input.charAt(0);

          if (Character.toLowerCase(lastComputerCityChar2) != Character.toLowerCase(firstInputChar)) {
      ...
      if (lastChar == 'ь' || lastChar == 'и' || lastChar == 'й') {
        char prevChar = input.charAt(input.length() - 2);
        lastComputerCity = getRandomCity(prevChar);
      } else {
